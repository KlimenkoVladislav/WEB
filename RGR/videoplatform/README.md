# 🎬 VideoHub — Тестовая видеоплатформа
...
🎬  VideoHub запущен!
   ...
   В сети:    откройте CMD и выполните: ipconfig
              используйте IPv4-адрес, например http://192.168.x.x:5000
```
...
Tailscale создаёт VPN-сеть между устройствами. Каждому участнику нужно установить приложение на tailscale.com (бесплатно). Даёшь свой Tailscale-адрес (например `http://100.x.x.x:5000`) — он работает как постоянная ссылка.
